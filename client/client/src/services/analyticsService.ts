...
// Use real API when available, fallback to mock for development
const USE_MOCK = import.meta.env.DEV && !import.meta.env.VITE_FORCE_REAL_API;
...
class AnalyticsService {
  ...
  private handleReconnect(onMessage: (event: AnalyticsWebSocketEvent) => void): void {
    // Disable reconnection in development
    if (import.meta.env.DEV) {
      return;
    }
