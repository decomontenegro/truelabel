 ...
 * Senior Engineering Approach: Smart fallback based on API availability
 */
export const shouldUseMock = (endpoint: keyof typeof ENDPOINT_CONFIG.MOCK_FALLBACK): boolean => {
  // Always use mock for endpoints that are not implemented in the API
  const apiEndpoints = ['auth', 'products', 'health', 'notifications'];
  const isApiEndpoint = apiEndpoints.includes(endpoint as string);

  // Use mock for non-API endpoints or when explicitly configured
  return !isApiEndpoint || ENDPOINT_CONFIG.MOCK_FALLBACK[endpoint];
};

/**
 * Smart service configuration - determines which services should use mock/fallback
 */
export const SERVICE_CONFIG = {
  // Services that should ALWAYS use mock (API not implemented)
  ALWAYS_MOCK: ['analytics', 'validationMetrics', 'websockets', 'realTimeAnalytics'],

  // Services that should try API first, fallback to mock on error
  API_WITH_FALLBACK: ['products', 'validations', 'reports', 'laboratories'],

  // Services that should only use real API
  API_ONLY: ['auth', 'notifications']
};
