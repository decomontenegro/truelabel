...
exports.getPlatformStats = exports.getProductsByCategory = exports.validateProduct = void 0;
const client_1 = require("@prisma/client");
const zod_1 = require("zod");
const prisma = new client_1.PrismaClient();
const validateProductSchema = zod_1.z.object({
    qrCode: zod_1.z.string().min(1, 'QR Code é obrigatório')
});
    ...
    try {
        ...
        await prisma.qRCodeAccess.create({
            data: {
                ...
                location: req.get('CF-IPCountry') || null
            }
        });
...
function calculateTrustScore(product) {
    ...
    const verifiedReports = product.reports.filter((r) => r.isVerified).length;
    score += Math.min(verifiedReports * 10, 30);
    let completenessScore = 0;
    if (product.description)
        completenessScore += 5;
    if (product.claims && Object.keys(product.claims).length > 0)
        completenessScore += 5;
    ...
    if (validValidations > 0) {
        const latestValidation = product.validations[0];
        const daysSinceValidation = Math.floor((Date.now() - new Date(latestValidation.validatedAt).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceValidation <= 30)
            ...
            score += 5;
        else if (daysSinceValidation <= 365)
            score += 3;
    }
...
const getPlatformStats = async (req, res) => {
    try {
        const [totalProducts, totalValidations, totalReports, totalBrands] = await Promise.all([
